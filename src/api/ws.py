...
async def stream_graph_to_sse(
    ...
    """Async generator yielding SSE-formatted strings from LangGraph.

    Drop-in alternative to stream_graph_to_ws for environments where
    WebSockets are blocked (e.g. HF Spaces). Uses plain HTTP streaming
    (text/event-stream) so it works through any HTTP/1.1 proxy.
    """
    ...
    try:
        async for event in graph.astream_events(initial_state, config=config, version="v2"):
            ...
            if kind == "on_chain_start" and name.endswith("_agent"):
                ...
                yield _sse({"type": "agent_start", "agent": agent})
                ...
                if ai_msg is not None:
                    ...
                    start_entry = (
                        trace_data["_starts"].get(parent_id)
                        or trace_data["_starts"].get(run_id)
                    )
                    elapsed = (
                        int((time.monotonic() - start_entry["t"]) * 1000)
                        if start_entry
                        else 0
                    )
                ...
                if isinstance(output, dict):
                    ...
                    for c in citations:
                        yield _sse({"type": "citation", **c})

    except Exception as exc:
        logger.error("graph sse streaming error", error=str(exc))
